...
class PsarcReader {
    ...
    fun read(raf: RandomAccessFile) {
        ...
        // ── Parse file entries with alignment correction ──────────────────
        for (i in 0 until numFiles.toInt()) {
            ...
            // FIX: Skip extra bytes if tocEntrySize > 30 to maintain alignment
            ...
            fun openStream(): InputStream {
                ...
                synchronized(raf) {
                    ...
                    while (output.size() < entry.length) {
                        // FIX: Safety check for corrupted or misaligned indices
