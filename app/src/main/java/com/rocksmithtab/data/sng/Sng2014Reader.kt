...
object Sng2014Reader {
    ...
    /**
     * Reads the SNG stream and automatically detects the platform for decryption.
     */
    fun read(stream: InputStream): Sng2014 {
        val raw = stream.readBytes()
        
        // Extract platform flags from the SNG header (offset 4-7)
        val platform = if (raw.size >= 8) {
            val buf = ByteBuffer.wrap(raw).order(ByteOrder.LITTLE_ENDIAN)
            val magic = buf.int and 0xFF
            if (magic == 0x4A) {
                val flags = buf.int
                // Bit 1 (0x2) indicates Mac/Console platform specific encryption
                if ((flags and 0x2) != 0) Platform.MAC else Platform.PC
            } else {
                Platform.PC
            }
        } else {
    ...
    private fun decrypt(data: ByteArray, platform: Platform): ByteArray {
        ...
        buf.int  // platform flags - skip (already used in read() for detection)
    ...
    private fun decompress(data: ByteArray): ByteArray {
        ...
        val buf = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN)
        buf.int                          // uncompressedLen
        val compressedLen   = buf.int
        
        if (data.size < 8 + compressedLen) return data
